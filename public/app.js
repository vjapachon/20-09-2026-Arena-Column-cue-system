...
function secondsUntil(targetTime, offsetMin = 0) {
  if (!targetTime || !targetTime.includes(':')) return -99999;
  ...
  target.setHours(h, m + offsetMin, s, 0);
...
// Formate "HH:MM" ou "HH:MM:SS" (garde les secondes si > 0)
...
function renderTimeline() {
  try {
    ...
    visibleCues.forEach((cue, idx) => {
      const startSec = parseTimeToSeconds(cue.start) || 0;
      ...
      if (nextCue) {
        const nextSec = parseTimeToSeconds(nextCue.start) || 0;
        endMin = nextSec / 60
               + (nextCue.offset || 0)
               + (state.settings?.globalOffset || 0);
        isAutoExtended = true;
      } else {
        endMin = startMin + (cue.duration || 30) / 60;
      }
...
function updateTopbarCounters() {
  ...
  let activeCue = null;
  todayCues.forEach(c => {
    const startSec = parseTimeToSeconds(c.start) + (c.offset || 0) * 60 + offsetSec;
    if (startSec === null) return;
    // duration en secondes maintenant
    const endSec = startSec + (c.duration || 0);
  ...
  const sorted = [...todayCues].sort((a, b) => {
    const aS = parseTimeToSeconds(a.start) || 0;
    const bS = parseTimeToSeconds(b.start) || 0;
    return aS - bS;
  });
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  bind('btnTestArena', () => testArena());

  // ─── Import Déroulé ───
  bind('btnImportDoc', () => openImportDocModal());
  bind('btnCloseImportDoc', () => document.getElementById('modalImportDoc')?.classList.add('hidden'));
  bind('btnAnalyzeDoc', () => analyzeImportDoc());
  bind('btnImportBack', () => backToImportStep1());
  bind('btnImportGenerate', () => generateImportCues());
...
async function openNetworkModal() {
  ...
  try {
    ...
    // ─── Charge l'IP Arena actuelle ───
    try {
      const arenaRes = await fetch('/api/arena/config');
      const arena = await arenaRes.json();
      const arenaEl = document.getElementById('arenaCurrent');
      if (arenaEl) arenaEl.textContent = `${arena.host}:${arena.port}`;
    } catch (e) {
      console.error('Erreur chargement Arena config:', e);
    }
  ...
  try {
    ...
    if (data.success) {
      ...
      data.results.forEach(r => {
        ...
        `;
        item.onclick = () => selectArena(r.ip, r.port);
...
// Sélectionne une Arena (config + test)
async function selectArena(host, port) {
  if (!confirm(`Configurer Arena sur ${host}:${port} ?\n\nLe scheduler utilisera cette adresse.`)) return;

  try {
    const res = await fetch('/api/arena/config', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
    ...
    if (data.success) {
      ...
      const arenaEl = document.getElementById('arenaCurrent');
      if (arenaEl) arenaEl.textContent = `${host}:${port}`;

      const statusEl = document.getElementById('arenaStatusText');
      if (statusEl) {
        statusEl.textContent = '✅ Connecté';
        statusEl.style.color = 'var(--green)';
      }

      alert(`✅ Arena configuré sur ${host}:${port}`);
    } else {
...
// Teste la connexion Arena actuelle
async function testArena() {
  ...
  try {
    ...
    // Test via l'API
    const testRes = await fetch(`/api/arena/config`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ host: arena.host, port: arena.port })
    });
...
let importParsedCues = [];

// Ouvre le modal d'import
function openImportDocModal() {
  const modal = document.getElementById('modalImportDoc');
  if (!modal) return;
  ...
  document.getElementById('btnAnalyzeDoc').disabled = true;
...
function setupImportDropzone() {
  ...
  const input = document.getElementById('importDocFile');
  ...
  input.onchange = onImportFileSelected;
}

function onImportFileSelected() {
  const input = document.getElementById('importDocFile');
  const dropzone = document.getElementById('importDropzone');
  const btn = document.getElementById('btnAnalyzeDoc');

  if (input.files.length > 0) {
    dropzone.classList.add('has-file');
    dropzone.querySelector('p strong').textContent = `✅ ${input.files[0].name}`;
    btn.disabled = false;
  } else {
...
// Analyse le fichier
async function analyzeImportDoc() {
  const input = document.getElementById('importDocFile');
  ...
  try {
    ...
    importParsedCues = data.cues || [];

    status.textContent = `✅ ${data.total} cue(s) détectée(s) depuis "${data.filename}"`;
...
function renderImportPreview() {
  ...
  if (importParsedCues.length === 0) {
    tbody.innerHTML = '<tr><td colspan="5" style="text-align:center;color:#8b949e;">Aucune cue détectée</td></tr>';
    return;
  }

  importParsedCues.forEach((cue, idx) => {
    const tr = document.createElement('tr');
    tr.innerHTML = `
      <td><input type="checkbox" checked data-idx="${idx}"></td>
      <td class="col-time">${cue.start}</td>
      <td class="col-name" title="${cue.name}">${cue.name}</td>
      <td class="col-duration">${cue.duration} min</td>
      <td class="col-column">${cue.column}</td>
    `;
    tbody.appendChild(tr);
  });
}
...
// Génère les cues
async function generateImportCues() {
  // Récupère les cues cochées
  const checkboxes = document.querySelectorAll('#importPreviewBody input[type="checkbox"]');
  const selectedCues = [];

  checkboxes.forEach(cb => {
    if (cb.checked) {
      const idx = Number(cb.dataset.idx);
      selectedCues.push(importParsedCues[idx]);
    }
  });

  if (selectedCues.length === 0) {
    alert('⚠️ Aucune cue sélectionnée.');
    return;
  }

  const mode = document.querySelector('input[name="importMode"]:checked')?.value || 'replace';

  if (!confirm(`Générer ${selectedCues.length} cue(s) en mode "${mode}" ?`)) return;

  try {
    const res = await fetch('/api/import/confirm', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ cues: selectedCues, mode })
    });

    const data = await res.json();

    if (data.success) {
      alert(`✅ ${data.count} cue(s) importée(s) !\nTotal : ${data.total}`);
      document.getElementById('modalImportDoc').classList.add('hidden');
    } else {
